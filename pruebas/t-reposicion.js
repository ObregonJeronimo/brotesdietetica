 ...
 * 1. Que anote cuando el stock sube y queda positivo, tambien en un alta con stock.
 * 2. Que no anote cuando baja, queda igual, sigue sin stock o se borra el producto.
 ...
 * 4. Que un producto borrado en el medio no sea un error.
 *
...
(async () => {
  ...
  t('cuando un negativo se corrige a 0: sigue sin mercaderia', (await escribir({ stock: -2 }, { stock: 0 })).escrituras.length === 0);
  ...
  FALLAR = Object.assign(new Error('14 UNAVAILABLE'), { code: 14 });
  t('otro error se registra y no rompe', (await escribir({ stock: 1 }, { stock: 2 })).error === null);
  FALLAR = null;
