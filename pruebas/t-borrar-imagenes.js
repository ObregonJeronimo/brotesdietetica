...
/* ------------------------------------------------------- el borrado, entero */
async function correr(prod, otros, falla) {
  const borrados = [];
  const fakes = {
  ...
  const fn = new Function(...nombres,
    ...
    cuerpo('imagenesDeProducto') + cuerpo('borrarImagenesDeProducto') +
    ';return borrarImagenesDeProducto;');
  const sacadas = await fn(...nombres.map(n => fakes[n]))(prod, prod.id);
